...
| `static-glob` | `StaticGlob` + shared path vocabulary; deprecate `RecursiveGlob` / `GlobFiles`; Filter path parity | Medium | **Landing** — core API + aliases + Filter parity + Methods docs; optional Glob wrapper remains — [`static-glob-rename.md`](design/plans/static-glob-rename.md) |
