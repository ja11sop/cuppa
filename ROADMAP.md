...
| `package-download-refresh` | Opt-in re-fetch of GitLab package archives (and re-extract) after same-version registry overwrite | Medium | Soak friction from **project D** / google-cloud-cpp stack; existence-only cache today. Design: [`package-download-refresh.md`](design/plans/package-download-refresh.md). Provisional CLI `--refresh-downloads` (names still open). |
| `package-build-publish-deps` | Opt-in cascade: from a tip publisher, topo-build and `--publish-package` each GitLab package dep then the tip | Medium | Needs `package_source` (or root) on dependency edges — package `develop=` is not publisher source. Design: [`package-build-publish-deps.md`](design/plans/package-build-publish-deps.md). **Defer until after #294.** Couples with download-refresh after each publish. |
| `package-use-libs-defaults` | Shared-aware `use_libs` + manifest `default_use_libs` so auto-enable can link small packages; explicit `use_libs` overrides (no link-the-world for fat packages) | Medium | **On branch** — shared-aware link + defaults + heuristic; design: [`package-use-libs-defaults.md`](design/plans/package-use-libs-defaults.md). |
| `package-metadata-amend` | Amend `cuppa-dependency.json` (and retar/publish) without rebuilding package binaries | Medium | google-cloud-cpp wants `default_use_libs=[]` without a multi-hour CMake rebuild. Design: [`package-metadata-amend.md`](design/plans/package-metadata-amend.md). |
...
| `parallel-job-count` | Optional `--parallel=N` (affinity + explicit job count); today’s workaround `--parallel --jobs=N` | Medium | Soak publishers want affinity without auto 14; [`parallel-job-count.md`](design/plans/parallel-job-count.md) |
