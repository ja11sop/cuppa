...
| `package-develop-local` | `--develop` on a package dependency means its **source tree**: build it locally and consume what it produced, with cascade and `--clone-develop` honouring the same path | Medium | Slices A–C shipped; post-C soak UX (nested argv, pending clone-develop, unused-develop note, package-source override, “this package” console copy) on this branch. Next: consuming a locally built package without the registry round trip (slice D). Design: [`package-develop-local.md`](design/plans/package-develop-local.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
