...
| `package-build-publish-deps` | Opt-in cascade: from a tip publisher, topo-build and `--publish-package` each GitLab package dep then the tip | Medium | Phase 1 MVP shipped in [#302](https://github.com/ja11sop/cuppa/pull/302) (`--build-and-publish-dependencies`, `--publisher-root`); Phase 2a in [#303](https://github.com/ja11sop/cuppa/pull/303) (`--cascade-plan`, nested session banners). Phase 2b in [#305](https://github.com/ja11sop/cuppa/pull/305): opt-in `--clone-publishers` clone of a publisher tree from a pinned `package_source` URL, into `<storage-root>/publishers` or `--publisher-root`. Next: honour per-dependency develop trees ahead of root lookup ([`package-develop-local.md`](design/plans/package-develop-local.md)), then skip-if-current. Design: [`package-build-publish-deps.md`](design/plans/package-build-publish-deps.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
| `package-develop-local` | `--develop` on a package dependency means its **source tree**: build it locally and consume what it produced, with cascade and `--clone-develop` honouring the same path | Medium | Proposal. Repairs a three-way disagreement: consume treats a package `develop=` as a built prefix, while `--list-develop` already inspects it as a working copy and `--clone-develop` lists it but has no URL to clone — which `package_source` now supplies. Design: [`package-develop-local.md`](design/plans/package-develop-local.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
