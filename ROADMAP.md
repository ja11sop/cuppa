...
| `package-build-publish-deps` | Opt-in cascade: from a tip publisher, topo-build and `--publish-package` each GitLab package dep then the tip | Medium | Phase 1 MVP shipped in [#302](https://github.com/ja11sop/cuppa/pull/302) (`--build-and-publish-dependencies`, `--publisher-root`); Phase 2a in [#303](https://github.com/ja11sop/cuppa/pull/303) (`--cascade-plan`, nested session banners). Phase 2b adds `--clone-publishers`: opt-in clone of a publisher tree from a pinned `package_source` URL, into `<storage-root>/publishers` or `--publisher-root`. Next: honour per-dependency develop trees ahead of root lookup, then skip-if-current. Design: [`package-build-publish-deps.md`](design/plans/package-build-publish-deps.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
