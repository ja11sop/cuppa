...
| `package-develop-local` | `--develop` on a package dependency means its **source tree**: discover (or opt-in stage) a local package and consume it, with cascade and `--clone-develop` honouring the same path | Medium | Slices A–D shipped ([#311](https://github.com/ja11sop/cuppa/pull/311) for D); post-C soak UX in [#310](https://github.com/ja11sop/cuppa/pull/310). D soak next; then slice E or [`stage-develop-locations`](design/plans/stage-develop-locations.md). Design: [`package-develop-local.md`](design/plans/package-develop-local.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
| `stage-develop-locations` | Extend `--stage-develop` to **location** develop trees that are Cuppa projects (nest build/clean); `--develop` alone stays in-tip compile | Medium | Proposal only. Design: [`stage-develop-locations.md`](design/plans/stage-develop-locations.md). Follows package half of `--stage-develop` in `package-develop-local`. |
