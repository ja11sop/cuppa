...
| `package-develop-local` | `--develop` on a package dependency means its **source tree**: build it locally and consume what it produced, with cascade and `--clone-develop` honouring the same path | Medium | Slices A and B shipped: develop paths anchor to the sconstruct directory, and cascade publishes from a develop tree (ranked first, refusing local-only work unless `--publish-modified-develop`) while the tip consumes what that publish produced. Next: `--clone-develop` for package dependencies, then consuming a locally built package without the registry. Repairs a three-way disagreement: consume treats a package `develop=` as a built prefix, while `--list-develop` already inspects it as a working copy and `--clone-develop` lists it but has no URL to clone — which `package_source` now supplies. Design: [`package-develop-local.md`](design/plans/package-develop-local.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
