...
| `package-build-publish-deps` | Opt-in cascade: from a tip publisher **or consume-only tip**, topo-build and `--publish-package` each GitLab package dep then tip publish **or tip build only** (`--publish-cascade-dependencies`) | Medium | Phase 1 MVP shipped in [#302](https://github.com/ja11sop/cuppa/pull/302) (`--build-and-publish-dependencies`, `--publisher-root`); Phase 2a in [#303](https://github.com/ja11sop/cuppa/pull/303) (`--cascade-plan`, nested session banners). Phase 2b in [#305](https://github.com/ja11sop/cuppa/pull/305): opt-in `--clone-publishers`. `--collect-cascade` / `--update-publishers` shipped. Phase **2c done** in [#323](https://github.com/ja11sop/cuppa/pull/323). Phase **2d done** in [#324](https://github.com/ja11sop/cuppa/pull/324) (single traveling `cuppa-publish.json`; stop writing `cuppa-dependency.json`; consume prefers publish with legacy fallback; amend removes twin). Decisions settled in [#322](https://github.com/ja11sop/cuppa/pull/322). Boost publish-seed `latest` **done** in [#328](https://github.com/ja11sop/cuppa/pull/328). Phase **3** consume-site parity **done** in [#329](https://github.com/ja11sop/cuppa/pull/329). Phase **4** pure-consume + `--publish-cascade-dependencies` on `feature/pure-consume-cascade`. Design: [`package-build-publish-deps.md`](design/plans/package-build-publish-deps.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
