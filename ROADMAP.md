...
| `doc-site-release-default` | Public docs default to **`/latest/`** (released Cuppa), not `master` tip | High | Naming settled; implement with `doc-llms-txt` — [`docs-site-release-default.md`](design/plans/docs-site-release-default.md) |
| `doc-llms-txt` | Agent Markdown: `llms.txt` + per-page MD + `llms-full.txt` from Antora HTML (Pandoc) | High | Same PR as release-default; corpus on `/latest/` — [`docs-llms-txt.md`](design/plans/docs-llms-txt.md) |
