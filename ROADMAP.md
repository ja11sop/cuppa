...
Cycle opened with docs/site and Profiles follow-ons; identity slices A–C closed the `tc-identity-coarsen` umbrella ([#243](https://github.com/ja11sop/cuppa/issues/243)). **`1.9.0`** is on PyPI / GitHub Releases (`v1.9.0`).
...
| `profiles-violation-report` | `prof-report-scope-filter` — method-only index filter (union of declaring sconscripts) | High | **Shipped** [#246](https://github.com/ja11sop/cuppa/pull/246) / [#205](https://github.com/ja11sop/cuppa/issues/205); [`cxx-profiles-report.md`](design/plans/cxx-profiles-report.md) §Collate index scope filter |
...
| See what is stored, where, and how large it is | Builds: `--list-builds` ([#140](https://github.com/ja11sop/cuppa/pull/140)); dependencies: `--list-dependencies` ([#141](https://github.com/ja11sop/cuppa/pull/141)), with lazy exact size upgrade ([#143](https://github.com/ja11sop/cuppa/pull/143)); downloads: `--list-downloads` ([#144](https://github.com/ja11sop/cuppa/pull/144)); `--list-scope` Option A (`all`/`resolve`/`referenced`/`unreferenced`/`compact`) on [#337](https://github.com/ja11sop/cuppa/pull/337) |
...
| `package-download-refresh` | Opt-in re-fetch of GitLab package archives (and re-extract) after same-version registry overwrite | Medium | Soak friction from **project D** / google-cloud-cpp stack; existence-only cache today. Design: [`package-download-refresh.md`](design/plans/package-download-refresh.md). Provisional CLI `--refresh-downloads` (names still open). [#296](https://github.com/ja11sop/cuppa/issues/296). |
| `package-build-publish-deps` | Opt-in cascade: from a tip publisher **or consume-only tip**, topo-build and `--publish-package` each GitLab package dep then tip publish **or tip build only** (`--publish-cascade-dependencies`) | Medium | Phase 1–4 + Q11 **done on master** ([#302](https://github.com/ja11sop/cuppa/pull/302)–[#333](https://github.com/ja11sop/cuppa/pull/333)): enable+companion actions, plan/collect/clone/update, skip-if-current, single `cuppa-publish.json`, consume-site `package_source`, pure-consume tips, tip `payload_sha256` overlay. Q7 collect reused/cloned **done on master** via [#334](https://github.com/ja11sop/cuppa/pull/334). Q1a `--list-publishers` / remove + develop chrome on [#335](https://github.com/ja11sop/cuppa/pull/335); **Q1b** (`--list-dependencies` publishers section) and **Q4** stem keying still open. Design: [`package-build-publish-deps.md`](design/plans/package-build-publish-deps.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
| `package-develop-local` | `--develop` on a package dependency means its **source tree**: discover (or opt-in stage) a local package and consume it, with cascade and `--clone-develop` honouring the same path | Medium | Slices A–D shipped ([#311](https://github.com/ja11sop/cuppa/pull/311) for D). Slice F (first-publish defer tip registry 404 under cascade) **shipped** [#316](https://github.com/ja11sop/cuppa/pull/316). Slice E (prefix migration) remains. Design: [`package-develop-local.md`](design/plans/package-develop-local.md) / [`cascade-defer-404.md`](design/archive/cascade-defer-404.md). [#297](https://github.com/ja11sop/cuppa/issues/297). |
...
| `integration-parallel-local` | Modest pytest-xdist for local integration gate (~2–3 min vs 8–10); isolate `HOME` first | Medium | Contributor tooling only; [`integration-parallel-local.md`](design/plans/integration-parallel-local.md). Prefer after `local-gate` orchestrator exists. |
| `local-gate` | `python -m scripts.local_gate`: venv re-exec, subprocess smoke preflight, then flake8 / pylint / unit / integration | Low | Stops agents burning a full integration run on env breaks (e.g. nested `six` import). Design: [`local-gate.md`](design/plans/local-gate.md). `impact:none`. |
...
| CMake-equivalent `GLOB_RECURSE` into one static lib | **Yes** — objects mirror under `working/` ([#213](https://github.com/ja11sop/cuppa/issues/213)); colliding archive basenames uniquified on `BuildStaticLib` ([#288](https://github.com/ja11sop/cuppa/pull/288)) |
...
| ID | Work | Priority | Notes |
|----|------|----------|-------|
| `method-behaviour-audit` | Fix output-path naming + document evaluation/returns taxonomy | High | **Shipped** — [#213](https://github.com/ja11sop/cuppa/issues/213) + glob parity + [#233](https://github.com/ja11sop/cuppa/issues/233) + cov nested-path sanity — [`method-behaviour-audit.md`](design/archive/method-behaviour-audit.md) |
| `doc-methods-split` | Hub + job-named method pages; engine APIs taught with Cuppa (not `scons-*` nav) | Medium | [`methods-pages-split.md`](design/plans/methods-pages-split.md); Methods baseline on [#234](https://github.com/ja11sop/cuppa/pull/234) |
| `doc-site-release-default` | Public docs default to **`/latest/`** (released Cuppa), not `master` tip | — | **Shipped** [#238](https://github.com/ja11sop/cuppa/pull/238) — verified 2026-09-10 (`/latest/` → **1.10.0**); [`docs-site-release-default.md`](design/archive/docs-site-release-default.md) |
| `doc-llms-txt` | Agent Markdown: `llms.txt` + per-page MD + `llms-full.txt` from Antora HTML (Pandoc) | — | **Shipped** [#238](https://github.com/ja11sop/cuppa/pull/238) — corpus on `/latest/`; [`docs-llms-txt.md`](design/archive/docs-llms-txt.md) |
...
| `doc-deps-four-hubs` | Dependencies: Using / Managing / Publishing / Authoring; Using-first usability | Medium | [`dependencies-docs-four-hubs.md`](design/plans/dependencies-docs-four-hubs.md) — P1–P4 on [#337](https://github.com/ja11sop/cuppa/pull/337); Publishing children split (GitLab / cascade / amend / Conan) |
