...
| ID | Work | Priority | Notes |
|----|------|----------|-------|
| `storage-listing-removal` | `--list-*`, `--remove-*`, `--purge-*` for builds, dependencies, and downloads | Medium | Builds done (#140); `--list-dependencies` done (#141); Slice D dependency removal done (#142 / §4.13); archive clean-by-variant (§4.14.3) done (#143); `--list-downloads` landing on `134_list_and_purge_downloads`; **next:** purge (Phase 4). GitHub [#134](https://github.com/ja11sop/cuppa/issues/134) |
