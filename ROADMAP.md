...
| `package-metadata-amend` | Amend `cuppa-dependency.json` (and retar/publish) without rebuilding package binaries | Medium | google-cloud-cpp wants `default_use_libs=[]` without a multi-hour CMake rebuild. Design: [`package-metadata-amend.md`](design/plans/package-metadata-amend.md). [#299](https://github.com/ja11sop/cuppa/issues/299). |
