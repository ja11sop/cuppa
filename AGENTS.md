...
`gcc`, `clang` + `--clang-stdlib=libstdc++`, and `clang` + `--clang-stdlib=libc++`, plus once on `windows-latest` with MSVC (`vc`), and a macOS modules slice that installs **Homebrew LLVM** Clang + libc++ (Apple/Xcode Clang is not modules-capable — no `clang-scan-deps`; see `docs/modules/ROOT/pages/cxx-modules.adoc` § macOS).
