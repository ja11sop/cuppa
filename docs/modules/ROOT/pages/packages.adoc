...
`env.Command`) for the CMake invocations. Pair with xref:methods/staging-files.adoc[Staging files]
...
| Map a dialect to an integer when CMake accepts it; experimental dialects (`c++2c`) may
  need a newer CMake or project-specific options
...
configure_cmd = (
    f'cmake -S . -B {cmake_out}'
    f' -D CMAKE_BUILD_TYPE={build_type}'
    f' -D CMAKE_INSTALL_PREFIX={install_prefix}'
    f' -D CMAKE_CXX_COMPILER={cxx}'
    f' -D BUILD_TESTING=OFF'
)
