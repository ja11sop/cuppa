...
After extract, call **`env.RemoveEmptyDirs`** with the project's download-target
directory names, then list the stamp among `CMakeConfigure`'s sources:
...
Or call `cuppa.buildsys.cmake.remove_empty_dirs(parent, names)` from your own
`Action` if you need a custom stamp. Only remove dirs that match that project's
download list; leave populated trees (and library submodules you satisfy via
`*_PROVIDER=package` / system packages) alone.
