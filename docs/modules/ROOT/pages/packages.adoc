...
==== Empty submodule placeholders (download archives)

GitHub source archives often leave **empty** directories for git submodules under
`third_party/`. Some upstream CMake trees only fetch those trees when the path
does **not** exist (for example gRPC's `gRPC_DOWNLOAD_ARCHIVES`). An empty
directory still exists, so configure fails later looking for files under it.
...
Or call `cuppa.buildsys.cmake.remove_empty_dirs(…)` from your own `Action`.
