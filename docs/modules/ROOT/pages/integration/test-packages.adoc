...
* ``GitlabPackagePublisher(..., dependencies=[...])`` writes ``cuppa-publish.json`` into the
  archive (transitive package metadata).
* Offline **alpha → beta → gamma** develop extracts: auto-enable only ``alpha``; transitive
  ``BuildWith`` supplies includes so a test that ``#include <alpha.hpp>`` compiles and runs.
