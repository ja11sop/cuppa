...
`--list-develop`) using ordinary warning / error colours for the prose. Already-deleted paths
are warnings; other failures are errors. Mixed success and failure under one rollup shows
`✓-✗`. Only bracketed values in the reason lines are coloured — for example `[Errno 13]` and
`[_build/lib/…/working]`.
