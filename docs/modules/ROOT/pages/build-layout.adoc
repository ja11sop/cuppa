...
| Generated reports and other artefacts outside the build tree | `_artifacts` | `--artefacts-root` (US alias: `--artifacts-root`)
...
To see built-in report kinds and their default directories without building:

[source,sh]
----
cuppa -D --list-reports
cuppa -D --list-reports --list-format=json
----

Full removal of artefact trees outside `_build/` (`--remove-artefacts`) is planned separately;
until then Profiles reports record paths in `.cuppa-reports` for matched `--clean` /
`--remove-builds`.
