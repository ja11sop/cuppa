...
shown through the documentation palette: it preserves Cuppa's semantic info, error, warning,
subdued, and emphasis roles while remaining readable on the site background.
...
Example output (colours approximate the terminal meanings):
...
`python -m scripts.generate_doc_samples list-builds`; the generator plants a generic build tree
and calls the same formatter as `--list-builds`. Add `--preview` to write a standalone page under
`_docs_build/samples/` while reviewing the semantic colours locally.
...
++++
include::partial$samples/remove-builds-dry-run.html[]
++++
...
++++
include::partial$samples/remove-builds-error.html[]
++++
...
++++
include::partial$samples/remove-all-builds-dry-run.html[]
++++
