...
C++ Profiles violation reports default to `<artefacts-root>/cxx-profiles/` when you pass
`--cxx-profiles-report` or declare `env.CxxProfilesReport()` in a sconscript.

=== Report kinds catalogue (`--list-report-kinds`)

Before you build — or when onboarding to a project — you may need to know **which HTML report
*kinds* cuppa can emit**, **where they would land** under the artefacts root, and **how to enable or
clean** each kind. `--list-report-kinds` answers that without running a compile. It lists **built-in
report types from cuppa's registry**, not files already present on disk:

[source,sh]
----
cuppa -D --list-report-kinds
cuppa -D --list-report-kinds --list-format=json
----

The listing names each built-in kind (Profiles violation inventory, coverage HTML, test HTML),
its default subdirectory when one exists, the CLI flags or `env.*()` method that produces it, and
how removal works today (for example Profiles paths recorded in `.cuppa-reports` for matched
`--clean`). It does **not** scan `_artefacts/` for existing index files, delete anything, or
replace a future on-disk report listing or `--remove-artefacts` (Phase 6 / #135).
