...
Before you build, you may need to know **which HTML report kinds cuppa can emit**, **which
toolchains on this system can produce each**, **where output would land** under the artefacts root,
and **how to enable or clean** each kind. `--list-available-reports` crosses the built-in report
registry with the same toolchain inventory as `--list-toolchains` (via ``supports_coverage()``,
``profiles_supported()``, test runners, and similar probes). It does **not** scan `_artefacts/` for
...
Pass `-Q` to suppress configure noise. Each kind lists supporting `--toolchains=` names (or
`(none on this system)`), CLI flags, sconscript methods, and clean behaviour today. It does **not**
replace a future on-disk report inventory or `--remove-artefacts` (Phase 6 / #135).
