...
# Custom
import cppscons.modules.registration
import cppscons.build_platform
import cppscons.output_processor
import cppscons.colourise
import cppscons.recursive_glob
import cppscons.configure
import cppscons.options

from cppscons.scms                   import *
from cppscons.toolchains             import *
from cppscons.methods                import *
from cppscons.dependencies           import *
from cppscons.profiles               import *
from cppscons.variants               import *
from cppscons.project_generators     import *
...
def add_base_options():
    ...
    SCons.Script.AddOption( '--projects', type='string', nargs=1,
                            action='callback', callback=cppscons.options.list_parser( 'projects' ),
...
class ParseToolchainsOption(object):
    ...
    def __call__(self, option, opt, value, parser):
        ...
        for toolchain in requested:
            ...
            if not supported:
                print "cppscons: requested toolchain(s) [{}] does not match any supported, skipping".format( toolchain )
            else:
                available = fnmatch.filter( self._available, toolchain )

                if not available:
                    print "cppscons: requested toolchain(s) [{}] supported does not match any available, skipping".format( toolchain )
                else:
                    toolchains.update( available )

        if not toolchains:
            print "cppscons: None of the requested toolchains are available"
...
class Construct(object):
    ...
    def get_option( cls, env, option, default=None ):
        ...
        if value:
            print "cppscons: option [{}] set {} as [{}]".format(
    ...
    def add_platforms( self, env ):
        ...
        env[platforms] = cppscons.build_platform.Platform.supported()


    def add_project_generators( self, env ):
        project_generators = self.project_generators_key
        env[project_generators] = {}
        cppscons.modules.registration.add_to_env( project_generators, { 'env': env } )


    def add_scm_systems( self, env ):
        scms = self.scm_systems_key
        env[scms] = {}
        cppscons.modules.registration.add_to_env( scms, { 'env': env } )
        ...
        cppscons.modules.registration.add_options( scms )
    ...
    def add_variants( self, env ):
        ...
        env[variants] = {}
        env[actions]  = {}
        cppscons.modules.registration.add_to_env( variants, { 'env': env } )
        cppscons.modules.registration.add_options( variants )
    ...
    def add_toolchains( self, env ):
        ...
        cppscons.modules.registration.add_to_env( toolchains, { 'env': env } )

        colouriser = env['colouriser']
        print "cppscons: supported toolchains are {}".format( colouriser.colour( 'notice', str( env["supported_toolchains"] ) ) )
        print "cppscons: available toolchains are {}".format( colouriser.colour( 'warning', str( env[toolchains].keys() ) ) )
    ...
    def initialise_options( self, env, default_options ):
        ...
        add_base_options()
        cppscons.modules.registration.add_options( self.toolchains_key )
        cppscons.modules.registration.add_options( self.scm_systems_key )
        cppscons.modules.registration.add_options( self.dependencies_key )
        cppscons.modules.registration.add_options( self.profiles_key )
        cppscons.modules.registration.add_options( self.project_generators_key )
    ...
    def print_construct_variables( self, env ):
        ...
        for key in keys:
            print "cppscons: Env[%s] = %s" % ( key, env[key] )
    ...
    def __init__( self,
        ...
        default_env['colouriser'] = cppscons.colourise.Colouriser.create()

        self._configure = cppscons.configure.Configure( default_env, callback=configure_callback )
        ...
        default_env['platform'] = cppscons.build_platform.Platform.current()
        ...
        cppscons.modules.registration.add_to_env( "dependencies",       { 'env': default_env } )
        cppscons.modules.registration.add_to_env( "profiles",           { 'env': default_env } )
        cppscons.modules.registration.add_to_env( "methods",            { 'env': default_env } )
        cppscons.modules.registration.add_to_env( "project_generators", { 'env': default_env } )
        ...
        if self._configure.handle_conf_only():
            print "cppscons: Handling onfiguration only, so no builds will be attempted."
            print "cppscons: With the current configuration executing 'scons -D' would be equivalent to:"
            print ""
            print "scons -D {}".format( self._command_line_from_settings( default_env['configured_options'] ) )
            print ""
            print "cppscons: Nothing to be done. Exiting."
            SCons.Script.Exit()
    ...
    def create_build_variants( self, toolchain, default_env ):
        ...
        for key, variant in active_variants.items():
            ...
            if not default_env['raw_output']:
                cppscons.output_processor.Processor.install( variant_envs[ key ] )
    ...
    def get_sub_sconscripts( self, path ):
        ...
        return cppscons.recursive_glob.glob(path, regex )
        ...
        if projects == None:
            ...
            if projects == None or not default_env['run_from_launch_dir']:
                ...
                if sub_sconscripts:
                    ...
                    print "cppscons: Using sub-sconscripts [{}]".format( self.colour_items( projects ) )
            elif projects != None:
                print "cppscons: Using default_projects [{}]".format( self.colour_items( projects ) )
        ...
        if projects:
            ...
            for project in projects:
                if os.path.exists( project ) and os.path.isdir( project ):
                    ...
                    if sub_sconscripts:
                        print "cppscons: Reading project folder [{}] and using sub-sconscripts [{}]".format(
            ...
            print "cppscons: No projects to build. Nothing to be done"
    ...
    def call_project_sconscript_files( self, toolchain, variant, env, project ):
        ...
        if os.path.exists( sconscript_file ) and os.path.isfile( sconscript_file ):
            ...
            print "cppscons: project exists and added to build [{}]".format( self._as_notice( sconscript_file ) )
            ...
            cppscons.modules.registration.init_env_for_variant( "methods", sconscript_exports )
            ...
            print "cppscons: Skipping non-existent project [{}]".format( self._as_error( sconscript_file ) )
