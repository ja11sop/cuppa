...
# Custom
import construct.modules.registration
import construct.build_platform
import construct.output_processor
import construct.colourise
import construct.recursive_glob
import construct.configure
import construct.options

from construct.scms                   import *
from construct.toolchains             import *
from construct.methods                import *
from construct.dependencies           import *
from construct.profiles               import *
from construct.variants               import *
from construct.project_generators     import *
...
def add_base_options():
    ...
    SCons.Script.AddOption( '--projects', type='string', nargs=1,
                            action='callback', callback=construct.options.list_parser( 'projects' ),
...
class Construct(object):
    ...
    def add_platforms( self, env ):
        ...
        env[platforms] = construct.build_platform.Platform.supported()


    def add_project_generators( self, env ):
        project_generators = self.project_generators_key
        env[project_generators] = {}
        construct.modules.registration.add_to_env( project_generators, { 'env': env } )


    def add_scm_systems( self, env ):
        scms = self.scm_systems_key
        env[scms] = {}
        construct.modules.registration.add_to_env( scms, { 'env': env } )
        ...
        construct.modules.registration.add_options( scms )
    ...
    def add_variants( self, env ):
        ...
        env[variants] = {}
        env[actions]  = {}
        construct.modules.registration.add_to_env( variants, { 'env': env } )
        construct.modules.registration.add_options( variants )
    ...
    def add_toolchains( self, env ):
        ...
        construct.modules.registration.add_to_env( toolchains, { 'env': env } )
    ...
    def initialise_options( self, env, default_options ):
        ...
        add_base_options()
        construct.modules.registration.add_options( self.toolchains_key )
        construct.modules.registration.add_options( self.scm_systems_key )
        construct.modules.registration.add_options( self.dependencies_key )
        construct.modules.registration.add_options( self.profiles_key )
        construct.modules.registration.add_options( self.project_generators_key )
    ...
    def __init__( self,
        ...
        default_env['colouriser'] = construct.colourise.Colouriser.create()

        self._configure = construct.configure.Configure( default_env, callback=configure_callback )
        ...
        default_env['platform'] = construct.build_platform.Platform.current()
        ...
        construct.modules.registration.add_to_env( "dependencies",       { 'env': default_env } )
        construct.modules.registration.add_to_env( "profiles",           { 'env': default_env } )
        construct.modules.registration.add_to_env( "methods",            { 'env': default_env } )
        construct.modules.registration.add_to_env( "project_generators", { 'env': default_env } )
    ...
    def create_build_variants( self, toolchain, default_env ):
        ...
        for key, variant in active_variants.items():
            ...
            if not default_env['raw_output']:
                construct.output_processor.Processor.install( variant_envs[ key ] )
    ...
    def get_sub_sconscripts( self, path ):
        ...
        return construct.recursive_glob.glob(path, regex )
    ...
    def call_project_sconscript_files( self, toolchain, variant, env, project ):
        ...
        if os.path.exists( sconscript_file ) and os.path.isfile( sconscript_file ):
            ...
            construct.modules.registration.init_env_for_variant( "methods", sconscript_exports )
