...
class AsciidocTemplateRunner(object):
    ...
    def __call__( self, target, source, env ):
        ...
        for t in targets:
            ...
            logger.debug( "AsciidocTemplateRunner: target = {}"
                          .format( as_info( str(t) ) ) )

        asciidoc_sources = []
        for s in sources:
            _is_asciidoc( str(s) ) and asciidoc_sources.append( s )
            logger.debug( "AsciidocTemplateRunner: source = {}"
...
class AsciidocTemplateEmitter(object):
...
class AsciidoctorToHtmlMethod(object):
    ...
    def __call__( self, env, target, source, final_dir=None, plantuml_config_file=None, template_file=None, css_file=None ):
        ...
        env.AppendUnique( BUILDERS = {
            'AsciidocTemplateToHtml' : env.Builder(
                action  = AsciidocTemplateRunner(
                        env,
                        plantuml_config=plantuml_config_file,
                        template_file=template_file,
                        css_file=css_file
                ),
                emitter = AsciidocTemplateEmitter(
                        plantuml_config=plantuml_config_file,
                        template_file=template_file,
                        css_file=css_file
                )
        ...
        html = env.AsciidocTemplateToHtml( target, source )
    ...
    def add_to_env( cls, cuppa_env ):
        cuppa_env.add_method( "AsciidocTemplateToHtml", cls() )
