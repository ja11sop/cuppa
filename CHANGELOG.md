...
- `--wipe-dependencies=name` clear-down of project-used extracts and matching downloads for the
  ...
  and `--force-wipe-unreferenced-dependencies`. List/storage type `location` renamed to
  `repository` (Python `location_dependency()` unchanged). Partial force-wipe reports keep
  unmatched same-identity siblings visible. Do not combine wipe modes with remove or purge (#146).
