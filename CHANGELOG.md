...
- ``--list-publishers`` matches ``--list-develop`` chrome: ruled STATUS table with
...
- Dependencies Antora map uses four job hubs (**Using** / **Managing** / **Publishing** /
  **Authoring**), with a Using-first usability page and Publishing split out of the old
  top-level packages megapage. Managing remove/wipe docs use intent + colourised command +
  Matches against a synthetic inventory (medium selectors; CLI copy toolbox); listing docs
  open with how to read the tree before GitLab ``requires``. Docs placeholders prefer
  ``<name>`` / ``<…>`` over Unicode ellipsis (``scripts.check_docs_placeholders``). Design:
...
- Public docs site serves the latest release under ``/cuppa/latest/…`` (Antora
  ...
  ([`docs-site-release-default.md`](design/archive/docs-site-release-default.md),
  [`docs-llms-txt.md`](design/archive/docs-llms-txt.md)).
...
- The documentation site keeps Antora's default UI bundle but overlays a Cuppa stylesheet
  ...
  tinted heading instead of reading as a grey patch. Four named palettes ship linked in
...
- Documentation subtree parents now have distinct landing and overview targets for Dependencies,
  ...
  listings get inset edge fades and click-drag pan. Dark-mode chrome (navbar end links,
  TOC ``Contents``, breadcrumbs/toolbar, burger, dropdowns) uses palette tokens instead of
  the bundle's light-theme hardcodes.
