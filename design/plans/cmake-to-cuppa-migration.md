...
- **Status:** proposal
- **Related:** [`ROADMAP.md`](../../ROADMAP.md) — `cmake-to-cuppa-migration`; [`archive/sconscript-exports.md`](../archive/sconscript-exports.md); [#213](https://github.com/ja11sop/cuppa/issues/213)
- **Updated:** 2026-09-01
...
| `GLOB_RECURSE` sources | `env.RecursiveGlob` (not SCons `env.Glob('**…')` — SCons Glob is not recursive) | Recursive snapshot walk; see [`recursive-glob-parity.md`](../archive/recursive-glob-parity.md) |
| `add_subdirectory` + target alias | **No export chain** between discovered sconscripts | [`sconscript-exports.md`](../archive/sconscript-exports.md) |
...
1. **Do not assume** `SConscript('child', exports=...)` works with Cuppa discovery — read [`sconscript-exports.md`](../archive/sconscript-exports.md).
2. **Do not assume** `RecursiveGlob` equals CMake `GLOB_RECURSE` evaluation semantics — static until re-configure; see [`recursive-glob-parity.md`](../archive/recursive-glob-parity.md). Check duplicate basenames (`find … -name '*.cpp' | xargs -n1 basename | sort | uniq -d`) — [#213](https://github.com/ja11sop/cuppa/issues/213).
