...
| Offline / remembered version | Same downloads-root scoping as source Boost `boost_latest_version`: shared `--downloads-root` → `~/.cuppaconfig`; downloads root under the project → project `configure.conf`. Store the concrete **version string** under a `gitlab_package_latest_*` key (suffix = hash of `registry\|package`, not an archive content hash). Online `"latest"` = newest in the registry now; offline = replay that remembered pin (or fail if none). Missing archive under this run’s downloads root fails — do not fall back to an older on-disk version. Do not scrape boost.org. Do not reuse the `boost_latest_version` key. |
| When to write the remembered version | After a successful Packages API list/select (current first cut). The value records “what latest resolved to,” not “archive proven on disk.” If download later fails, offline still pins that version and then fails when the archive is absent — that is the intended contract (cannot honour latest), not a silent older fallback. Stricter “only remember after cache hit” (as source Boost) is an optional follow-on, not a change to the offline contract. |
...
| `pkg-latest-issue` | Done — [#271](https://github.com/ja11sop/cuppa/issues/271) |
