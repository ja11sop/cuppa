...
| [`plans/package-develop-local.md`](plans/package-develop-local.md) | in progress | `--develop` for a package dependency means its source tree: discover or `--stage-develop` nest-stage; cascade and `--clone-develop` honour it — [#297](https://github.com/ja11sop/cuppa/issues/297) |
| [`plans/stage-develop-locations.md`](plans/stage-develop-locations.md) | in progress | Extend `--stage-develop` to location develop trees that are Cuppa projects (nest build/clean); keep `--develop` alone as in-tip compile — ROADMAP `stage-develop-locations` |
