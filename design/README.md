...
| [`plans/stage-develop-locations.md`](plans/stage-develop-locations.md) | proposal | Extend `--stage-develop` to location develop trees that are Cuppa projects (nest build/clean); keep `--develop` alone as in-tip compile — ROADMAP `stage-develop-locations` |
