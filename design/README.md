...
| [`plans/cxx-profiles-report.md`](plans/cxx-profiles-report.md) | in progress | `--cxx-profiles-report`: classify/dedupe Profiles diagnostics — ROADMAP `profiles-violation-report` / **1.8.0** — **E** + **F-min** on branch `prof-report-method-artefacts-min`; next **`prof-report-method-semantics`** |
